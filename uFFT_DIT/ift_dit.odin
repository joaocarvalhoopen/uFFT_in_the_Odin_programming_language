...
@(private="file")
nop_split :: proc ( x : [ ^ ]complex64 , X : [ ^ ]complex64, N : u32 ) {
...
// {
// 	for (size_t n = 0; n < N/2; n++) {
// 		X[0/2 + n] = (x[2*n + 0] + x[2*n + 1] * cexpf(+2*(float)M_PI*I*phi))/2;
// 		X[N/2 + n] = (x[2*n + 0] - x[2*n + 1] * cexpf(+2*(float)M_PI*I*phi))/2;
// 	}
// }
...
fft_split :: proc ( x : [ ^ ]complex64, X : [ ^ ]complex64, N : u32, phi : complex64 ) {
	for n : u32 = 0; n < N / 2; n += 1 {
		X[ 0 / 2 + n ] = ( x[ 2 * n + 0 ] + x[ 2 * n + 1 ] * cmplx.exp_complex64( complex64( complex( 0, +2 * f32( math.PI ) ) ) * phi ) ) / 2
		X[ N / 2 + n ] = ( x[ 2 * n + 0 ] - x[ 2 * n + 1 ] * cmplx.exp_complex64( complex64( complex( 0, +2 * f32( math.PI ) ) ) * phi ) ) / 2
	}
}
...
@(private="file")
revbits :: proc ( v : u32, J : i32 ) -> i32 {
	r : u32 = 0

	for j : i32 = 0; j < J; j += 1 {
		r |= ( ( v >> uint( j ) ) & 1 ) << uint( J - 1 - j )
	}
...
nop_reverse :: proc ( b : i32, /* float complex *buffers[2] */ buffers : [ 2 ][ ^ ]complex64 , N : u32 ) -> i32 {
	...
	for j : i32 = 0; j < J - 1; j += 1 /* , b++ */ {
		delta : u32 = N >> uint( j )

		for n : i32 = 0; n < i32( N ); n += i32( delta ) {
			buf_a := mem.ptr_offset( buffers[ b & 1 ], n )
            buf_b := mem.ptr_offset( buffers[ ~b & 1 ], n )
            nop_split( buf_a, buf_b, delta )
		}
        b += 1
	}
...
fft_reverse :: proc ( b : i32, /* float complex *buffers[2] */ buffers : [ 2 ][ ^ ]complex64, N : u32 ) -> i32 {
	...
	for j : i32 = 0; j < J; j += 1 /* , b++ */ {
		...
		for n : i32 = 0; n < i32( N ); n += i32( delta ) {
			phi : complex64 = complex64( complex(  ( ( transmute( f32 ) revbits( u32( n ) / delta, j ) ) / ( transmute( f32 ) ( u32( 2 ) << uint( j ) ) ) ), 0 ) )
			buf_a := mem.ptr_offset( buffers[ b & 1 ], n )
            buf_b := mem.ptr_offset( buffers[ ~b & 1 ], n )
...
ift :: proc ( vector : [ ^ ]complex64, N :  int ) -> int {
	if N <= 0 {
        return 0
    }
